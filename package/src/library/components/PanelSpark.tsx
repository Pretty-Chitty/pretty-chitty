...
export default function PanelSpark({
  chit,
  paused,
}: {
  chit: SparkChit;
  ...
  const sparkChit = useChit<SparkChit>(chit._id ?? "no id");

  const [value, setValue] = useState(
    sparkChit?.value ?? Number.MIN_SAFE_INTEGER
  );
  ...
  useEffect(() => {
    if (targetValue !== value) {
      if (paused) {
        chit.parent?._renderInstance?.rootRenderInstance.markHasPendingChange();
        return;
      }

      setValue(targetValue);
      if (
        targetValue !== Number.MIN_SAFE_INTEGER &&
        value !== Number.MIN_SAFE_INTEGER
      ) {
        const key = `spark${chit._id ?? "no id"}`;
  ...
  const image =
    chit.ZZZicon instanceof PlayerChit
      ? new PlayerCanvas(HEIGHT * 3, HEIGHT * 3, chit.ZZZicon).get()
      : new IconCanvas(HEIGHT * 3, HEIGHT * 3, chit.ZZZicon).get();

  const color =
    (chit.color.length > 0 ? chit.color : chit.ZZZicon?.color) ?? "#ffffff";
  ...
  return (
    <Box
      ...
      sx={{
        ...
        transition: flashed
          ? "background linear 0.02s"
          : `background linear ${DURATION / 1000}s`,
