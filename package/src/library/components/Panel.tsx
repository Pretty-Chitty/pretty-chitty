...
function ViewerWrapper({
  ...
  const chitInstance = useChit(chit._id ?? "nochit");
  ...
  return (
    <>
      <Stack
        ...
        sx={{ position: "absolute", zIndex: ZINDEX_SPARKS }}
      >
        {sparks
          .concat()
          .reverse()
          .map((spark) => (
            <PanelSpark key={spark._id} chit={spark} paused={paused} />
          ))}
      </Stack>
      <Viewer
        paused={paused}
...
function MultiPanel({
  ...
  const [targetAnimationSpeed] = useEventChannelState(
    timeState.targetAnimationSpeedMultiplier
  );
  const [actualAnimationSpeed] = useEventChannelState(
    timeState.animationSpeedMultiplier
  );
  const [selectedIndex, setSelectedIndex] = useDebounce(0);
  const rootRenders = chits.map((c) =>
    c._renderInstance instanceof RootChitRenderInstance
      ? c._renderInstance
      : undefined
  );
  ...
  const key = `panel--${chits.map((c) => c._id).join("-")}`;
  useEffect(() => {
    setIsSliding(true);
          ...
          <Box
            ...
            sx={{
              ...
              transform:
                index === selectedIndex
                  ? `translateX(0)`
                  : `translateX(${index > selectedIndex ? "110%" : "-110%"})`,
            }}
          >
            <ViewerWrapper
              paused={
        ...
        >
          ...
          <Box
            sx={{
              height: CUTOUT_HEIGHT,
              width: "100%",
              background: theme.panelSelectionCutoutBackground,
            }}
          />
          <Box
            sx={{
