...
export class SelectPrompt extends Prompt {
  ...
  serializeDetails(): any {
    return this.chits.map((chit) => chit._id);
  }
  ...
  resolveDetails(chitId: string) {
    this.resolved = true;
    this.selectedChit = this.chits.find((chit) => chit._id === chitId);
  ...
  stageIn() {
    this.chits.forEach((c) => (c.onClick = () => this.resolve(c._id)));
  }
...
export class PickPrompt extends Prompt {
  ...
  deserializeDetails(details: {
    ...
    this.picks = details.picks.map((d) =>
      Pick.deserialize(d, this.findChit, this.buttonLibrary)
    );
