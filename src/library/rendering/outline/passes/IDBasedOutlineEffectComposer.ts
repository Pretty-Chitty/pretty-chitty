...
import { IDBasedMaskPass } from "./IDBasedMaskPass";
import { IDBasedEdgeDetectionPass } from "./IDBasedEdgeDetectionPass";
import { BlurPass, BlurDirection } from "./BlurPass";
import { OutlineCompositePass } from "./OutlineCompositePass";

export class IDBasedOutlineEffectComposer extends Pass {
  renderScene: Scene;
  renderCamera: Camera;
  ...
  private idBasedMaskPass!: IDBasedMaskPass;
  private downsamplePass!: ShaderPass;
  private edgeDetectionPass!: IDBasedEdgeDetectionPass;
  ...
  private initializeRenderTargets(): void {
    ...
    this.renderTargetMaskBuffer.texture.name = "IDBasedOutline.mask";
    this.renderTargetMaskBuffer.texture.generateMipmaps = false;

    this.renderTargetMaskDownSampleBuffer = new WebGLRenderTarget(resx, resy, pars);
    this.renderTargetMaskDownSampleBuffer.texture.name = "IDBasedOutline.maskDownSample";
    this.renderTargetMaskDownSampleBuffer.texture.generateMipmaps = false;

    this.renderTargetEdgeBuffer1 = new WebGLRenderTarget(resx, resy, pars);
    this.renderTargetEdgeBuffer1.texture.name = "IDBasedOutline.edge1";
    this.renderTargetEdgeBuffer1.texture.generateMipmaps = false;

    this.renderTargetBlurBuffer1 = new WebGLRenderTarget(resx, resy, pars);
    this.renderTargetBlurBuffer1.texture.name = "IDBasedOutline.blur1";
    this.renderTargetBlurBuffer1.texture.generateMipmaps = false;

    this.renderTargetBlurBuffer2 = new WebGLRenderTarget(Math.round(resx / 2), Math.round(resy / 2), pars);
    this.renderTargetBlurBuffer2.texture.name = "IDBasedOutline.blur2";
    this.renderTargetBlurBuffer2.texture.generateMipmaps = false;

    this.renderTargetEdgeBuffer2 = new WebGLRenderTarget(Math.round(resx / 2), Math.round(resy / 2), pars);
    this.renderTargetEdgeBuffer2.texture.name = "IDBasedOutline.edge2";
  ...
  render(
    ...
    // Execute the ID-based pipeline
    this.executeIDRenderPass(renderer);
    this.executeIDMaskPass(renderer);
  ...
  private executeIDMaskPass(renderer: WebGLRenderer): void {
    this.idBasedMaskPass.setIDTexture(this.renderTargetIDBuffer.texture);
    this.idBasedMaskPass.setSelectedObjects(this.selectedObjects);
    this.idBasedMaskPass.render(renderer, this.renderTargetMaskBuffer);
  }
  ...
  private executeCompositePass(renderer: WebGLRenderer, readBuffer: WebGLRenderTarget, maskActive: boolean): void {
    ...
    // For the composite, we can use the original mask buffer or the ID buffer for more precise masking
