...
const LINE_COLOR = new MeshBasicMaterial({
  color: 0xff0000,
  wireframe: true,
  wireframeLinewidth: 2,
});
const CLICK_LINE_COLOR = new MeshBasicMaterial({
  color: 0xffff00,
  wireframe: true,
  wireframeLinewidth: 2,
});
...
export class ChitRenderInstance {
  ...
  constructor(public chit: Chit) {
    ...
    if (
      chit._renderInstance &&
      chit.parent?._renderInstance?.rootRenderInstance
    ) {
      const intersection = this.attemptToFindPlaneZ0(
        chit.parent?._renderInstance?.rootRenderInstance,
        chit
      );
      if (intersection) {
  ...
  public init() {
    this.chit._children.forEach((child) => {
      if (child.canRender()) {
        if (!child._renderInstance) {
          const c = new ChitRenderInstance(child);
          c.init();
        } else {
          child._renderInstance.refresh();
        }
      }
    });
  ...
  public childAdded(chit: Chit, existingRenderInstance?: ChitRenderInstance) {
    if (
      existingRenderInstance?.rootRenderInstance === this.rootRenderInstance
    ) {
      return;
    }
  ...
  public invalidateRootRenderInstance() {
    ...
    this.childrenRenderInstances.forEach((child) =>
      child.invalidateRootRenderInstance()
    );
  }
  ...
  public get rootRenderInstance(): RootChitRenderInstance {
    ...
    return (this._rootRenderInstance =
      this.parentRenderInstance?.rootRenderInstance);
  }
  ...
  protected setOutletPosition(positionKey: string, position: Vector3) {
    ...
    this.bboxAnchor(positionKey).position.set(
      position.x,
      position.y,
      position.z
    );
  ...
  protected updateGroupPosition(
    group: Group,
    position: OwnerOriginPosition | string
  ) {
  ...
  public destroy() {
    ...
    if (this.chit._renderInstance === this) {
      this.chit._renderInstance = undefined;
    }
  }
  ...
  protected refresh() {
    ...
    if (isVisible && this.renderSpec) {
      ...
      this.renderSpec.ornaments.forEach((ornament) =>
        this.group.remove(ornament)
      );
    }
    ...
    this.innateOrnamentZs = this.renderSpec.ornaments.map(
      (o) => o.position?.z ?? 0
    );
  ...
  public screenCoordinates(): Vector2 | undefined {
    ...
    const screenCoords =
      this.rootRenderInstance.convertCameraSpaceToScreenSpace(
        vector.x,
        vector.y
      );
    return screenCoords;
  ...
  protected attemptToFindPlaneZ0(
    ...
    if (
      rootRenderInstance &&
      rootRenderInstance.rootGroup &&
      screenCoordsOfNewLocation
    ) {
  ...
  protected updateBoundingBox() {
    ...
    if (keyChanged) {
      ...
      [...this.anchorPoints.entries()].forEach(([key, value]) =>
        this.updateGroupPosition(value, key)
      );
      [...this.bboxAnchorPoints.entries()].forEach(([key, value]) =>
        this.updateGroupPosition(value, key)
      );
      ...
      this.clickbox.position.z =
        (clickBox3.max.z - clickBox3.min.z) / 2 +
        clickBox3.min.z +
        this.sizeZ / 2;
      this.clickbox.position.x =
        (clickBox3.max.x - clickBox3.min.x) / 2 + clickBox3.min.x;
      this.clickbox.position.y =
        (clickBox3.max.y - clickBox3.min.y) / 2 + clickBox3.min.y;

      const targetOffset = {
        x: this.renderSpec.offsetX,
        y: this.renderSpec.offsetY,
        z: this.renderSpec.offsetZ,
      };

      this.handleOffsetForSplay(targetOffset);

      this.bboxGroup.position.set(
        targetOffset.x,
        targetOffset.y,
        targetOffset.z
      );
      this.bboxGroup.rotation.set(
        this.renderSpec.rotateX,
        this.renderSpec.rotateY,
        this.renderSpec.rotateZ
      );

      Object.entries(this.renderSpec.outletPositions).forEach(
        ([key, position]) => {
          this.setOutletPosition(
            key,
            new Vector3(position.x, position.y, position.z + this.sizeZ)
          );
        }
      );
  ...
  protected removeChild(child: ChitRenderInstance) {
    this.childrenRenderInstances = this.childrenRenderInstances.filter(
      (d) => d !== child
    );
  }
  ...
  protected handleHierarchy() {
    const targetParentRenderInstance = this.chit.parent?._renderInstance;
    ...
    const targetParentGroup =
      (this.renderSpec && this.parentRenderInstance?.anchor(origin)) ??
      this.rootGroup;
    const targetParentBboxGroup =
      (this.renderSpec && this.parentRenderInstance?.bboxAnchor(origin)) ??
      this.rootGroup;
  ...
  private handleOffsetForSplay(p: Point3d) {
    ...
    if (
      this.chit.parentOutlet &&
      this.chit.parentOutletIndex !== undefined &&
      this.renderSpec.splay.enabled
    ) {
  ...
  protected handlePositionAndRotation() {
    ...
    const targetRotation = {
      x: this.renderSpec.rotateX,
      y: this.renderSpec.rotateY,
      z: this.renderSpec.rotateZ,
    };
    ...
    if (
      ...
      offsetEasing = this.offsetTween.isPlaying()
      ...
      duration = Math.max(
        duration,
        this.renderSpec.offsetSpeed *
          Math.min(this.renderSpec.maxDistanceForSpeed, distanceMoved)
      );
    }
    ...
    if (
      ...
      rotationEasing = this.rotationTween.isPlaying()
        ? Easing.Quadratic.Out
        : Easing.Quadratic.InOut;
      this.rotationTween.stop();
      ...
      const nonZRadiansDistance = new Quaternion()
        ...
        .angleTo(
          new Quaternion().setFromEuler(
            new Euler(targetRotation.x, targetRotation.y, 0)
          )
        );
      nonZRotations = Math.min(
        nonZRadiansDistance / (2 * Math.PI),
        2 * Math.PI
      );
    ...
    if (offsetEasing || rotationEasing) {
      ...
      const peak = {
        z:
          ...
          Math.max(
            distanceMoved * zLiftRatio,
            nonZRotations * this.renderSpec.zLiftRotationMultiplier
          ),
      };
        ...
        tween
          ...
          .chain(
            new Tween({ z: peak.z }, this.tweenGroup)
              .to(
                { z: targetOffset.z },
                (duration / 2) * this.animationSpeedMultiplier
              )
              .easing(Easing.Quadratic.In)
              .onUpdate((obj) => {
                position.z = obj.z;
  ...
  protected handleRotation() {
    ...
    if (
      ...
      const easing = this.rotationTween
        ? Easing.Quadratic.Out
        : Easing.Quadratic.InOut;
      ...
      this.rotationTween = this.createTween(rotation, (tween) =>
        tween
          .to(
            target,
            renderSpec.rotationSpeed * rotations * this.animationSpeedMultiplier
          )
          .easing(easing)
      );
    }
